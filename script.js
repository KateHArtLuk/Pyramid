...
function generateBox(x, y, z, width, depth, falls) {
    ...
    const color = new THREE.Color(`hsl(${300 + stack.length * 4}, 90%, 50%)`);
...
function init() {
    ...
    const dirLight = new THREE.DirectionalLight(0xfff000, 0.5);
...
// своя обработка нажатия пробела
...
function splitBlockAndAddNextOneIfOverlaps() {
    ...
    if (overlap > 0) {
        ...
        const nextDirection = direction == "z" ? "x" : "z";

        // если идёт подсчёт очков — выводим текущее значение
        if (scoreElement) scoreElement.innerText = stack.length - 1;
        if (scoreElement.innerHTML === '20') {
            ...
            if (!autopilot) winElement.style.display = "flex";
...
// выводим нужное сообщение и обрабатываем
...
function updatePhysics(timePassed) {
    ...
    world.step(timePassed / 800); // Step the physics world
